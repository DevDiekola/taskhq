...
import { TASK_LOCAL_STORAGE_KEY, TABLE_VIEW_LOCAL_STORAGE_KEY, KANBAN_VIEW_LOCAL_STORAGE_KEY, TASK_SLICE_NAME } from '@/constants/task';
import { RootState } from '../reducers/root';

const taskStorageMiddleware: Middleware<object, RootState> = storeAPI => next => action => {
    const result = next(action);

    if (!(action as Action).type.startsWith("task/") && (action as PayloadAction<string>)?.payload !== TASK_SLICE_NAME) {
        return;
    }
